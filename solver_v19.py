"""
AutoSolver v19: 低意愿场景深度优化
====================================
v18平台分719.29，low_willingness仍高达1803
核心改进：
1. 低意愿时primary选择用更高penalty（让per_task_cost更倾向高w骑手）
2. 低意愿时backup选择优先高willingness骑手（降低p_rej最有效）
3. 增加willingness优先的primary策略
4. ALNS repair也优先高willingness
"""
...
def _solve_main(input_text: str) -> list:
    ...
    for primary_strategy in strategies:
        for penalty in penalties:
            ...
            # v19: backup允许骑手出现在多个slot中
            assignments = _add_backups_multi(primary, all_couriers, taskkey_to_cands, cand_map, penalty)
            # local_swap也允许跨slot共享骑手
            assignments = _local_swap_multi(assignments, taskkey_to_cands, cand_map, 100)
    ...
    if not best_assignments or len(baseline_covered) > best_covered:
        ...
        assignments = _add_backups_multi(primary_from_baseline, all_couriers, taskkey_to_cands, cand_map, 100)
        assignments = _local_swap_multi(assignments, taskkey_to_cands, cand_map, 100)
        best_assignments = assignments
...
def _pick_primary_willingness_first(candidates, all_tasks, penalty):
    """willingness优先策略：按willingness降序选骑手，同willingness按per_task_cost升序。
    在低意愿场景下，高willingness骑手比低score骑手更重要（降低p_rej最有效）。
    """
    enriched = []
    for c in candidates:
        n = len(c["task_ids"])
        w = c["willingness"]
        per_task_cost = (w * c["score"] + (1 - w) * penalty * n) / n
        # 先按willingness降序，再按per_task_cost升序
        enriched.append(((-w, per_task_cost), c))
    enriched.sort(key=lambda x: x[0])
    used_c = set()
    used_t = set()
    out = []
...
def _pick_primary_concentrate_high_w(candidates, all_tasks, penalty, taskkey_to_cands, cand_map):
    """集中高w骑手策略：优先给候选质量差的task_key分配最好的骑手。
    
    核心思想：在低意愿场景下，某些task_key只有低willingness的候选骑手。
    这些task_key的p_rej很高，是成本的主要来源。
    应该优先给这些task_key分配高willingness骑手（如果有的话），
    而不是把高w骑手浪费在已有好候选的task_key上。
    """
    # 1. 计算每个task_key的"最佳可用willingness"
    tk_best_w = {}
    for tk, cands in taskkey_to_cands.items():
        best_w = max((c["willingness"] for c in cands), default=0)
        tk_best_w[tk] = best_w
    
    # 2. 按task_key的最佳willingness升序排列（最差的优先分配）
    # 但需要考虑task_key覆盖的任务
    task_to_tks = defaultdict(list)
    for tk in taskkey_to_cands:
        # 从task_key解析任务
        tasks = tk.split(",")
        for t in tasks:
            task_to_tks[t.strip()].append(tk)
    
    # 3. 按task的候选质量排序（最差的优先）
    task_difficulty = {}
    for t in all_tasks:
        tks = task_to_tks.get(t, [])
        # 该任务可用的最佳willingness
        best_w = max((tk_best_w.get(tk, 0) for tk in tks), default=0)
        task_difficulty[t] = -best_w  # 越难的任务越先分配
    
    # 4. 按难度排序任务，然后贪心分配
    sorted_tasks = sorted(all_tasks, key=lambda t: (task_difficulty[t], t))
    ...
    for t in sorted_tasks:
        ...
        for tk in tks:
            # 检查该task_key的所有任务是否都未被使用
            tk_tasks = [tt.strip() for tt in tk.split(",")]
            if any(tt in used_t for tt in tk_tasks):
                continue
            
            # 在该task_key的候选中选最好的骑手
            for c in taskkey_to_cands[tk]:
                ...
                w = c["willingness"]
                per_task_cost = (w * c["score"] + (1 - w) * penalty * n) / n
                if per_task_cost < best_cost:
                    best_cost = per_task_cost
                    best_choice = (tk, c)
        
        if best_choice:
            tk, c = best_choice
            out.append((tk, c["courier_id"]))
            used_c.add(c["courier_id"])
            for tt in c["task_ids"]:
                used_t.add(tt)
...
def _add_backups(primary, all_couriers, taskkey_to_cands, cand_map, penalty, aggressive=False):
    ...
    v19: aggressive时优先选高willingness骑手作为backup。
    """
    assignments = []
    used_couriers = set()
    for tk, cid in primary:
...
# ============================================================
# Multi-slot函数（允许骑手出现在多个slot中）
# ============================================================

def _add_backups_multi(primary, all_couriers, taskkey_to_cands, cand_map, penalty):
    """添加backup骑手，允许骑手出现在多个slot中。
    核心突破：高willingness骑手可以同时作为多个slot的backup，
    大幅降低每个slot的p_rej。
    """
    assignments = []
    for tk, cid in primary:
    ...
    # 为每个slot独立添加backup，不限制骑手跨slot使用
    max_per_slot = 4  # 每个slot最多4个骑手
    min_p_rej = 0.005

    for idx, (tk, cids) in enumerate(assignments):
        for _ in range(max_per_slot - 1):  # 最多添加3个backup
            p_rej = 1.0
            for cid in cids:
                cc = cand_map.get((tk, cid))
                if cc:
            ...
            for c in taskkey_to_cands[tk]:
                cid = c["courier_id"]
                # 不检查used_couriers！允许跨slot使用
                if cid in cids:  # 只避免同一slot内重复
                    continue
                new_cids = cids + [cid]
                new_cost = _slot_cost(tk, new_cids, cand_map, penalty)
                gain = current_cost - new_cost
...
def _local_swap_multi(assignments, taskkey_to_cands, cand_map, penalty):
    """局部搜索，允许骑手出现在多个slot中。"""
    improved = True
    rounds = 0
    while improved and rounds < 5:
        ...
        for i, (tk, cids) in enumerate(assignments):
            ...
            for pos in range(len(cids)):
                ...
                for c in taskkey_to_cands[tk]:
                    new_cid = c["courier_id"]
                    if new_cid == old_cid or new_cid in cids:
                        continue
                    cids[pos] = new_cid
                    new_cost = _slot_cost(tk, cids, cand_map, penalty)
                    if new_cost < current_cost - 1e-6:
...
def _inter_swap_multi(assignments, all_couriers, taskkey_to_cands, cand_map, penalty):
    """跨slot交换，允许骑手出现在多个slot中。
    由于骑手可以跨slot使用，主要操作是：
    1. 为每个slot尝试添加新骑手
    2. 从每个slot尝试删除冗余骑手
    3. 在两个slot之间交换骑手
    ...
    while improved and rounds < 5:
        ...
        for j in range(n_asgn):
            ...
            best_gain = 0.0
            best_X = None
            for c in taskkey_to_cands[tk_j]:
                X = c["courier_id"]
                if X in cids_j:
                    continue
                cids_j.append(X)
...
def _alns_multi(initial_assignments, candidates, all_tasks, all_couriers,
                cand_map, taskkey_to_cands, penalty, deadline, is_low_w=False):
    """ALNS，允许骑手出现在多个slot中。"""
    import math, random, time
    ...
    while time.time() < deadline - 0.2:
        ...
        _repair_greedy(cand, candidates, all_tasks, cand_map, penalty)
        # multi版backup：不限制骑手跨slot使用
        _add_backups_light_multi(cand, taskkey_to_cands, cand_map, penalty)
        cand = _inter_swap_multi(cand, all_couriers, taskkey_to_cands, cand_map, penalty)
...
def _add_backups_light_multi(assignments, taskkey_to_cands, cand_map, penalty):
    """轻量backup（multi版），为高p_rej的slot添加backup"""
    max_per_slot = 4
    min_p_rej = 0.005
