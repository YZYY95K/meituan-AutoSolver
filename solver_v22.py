"""
AutoSolver v22: 基于MIECP论文的碰撞感知优化
====================================
v21平台分719.05
核心改进（基于"Recommend-to-Match with Random Supply Rejections"论文）：
1. 指数锥近似：用log(1-prod(1-w_i))近似评估slot成本，比线性近似更准确
2. 碰撞感知primary选择：优先选择"独占"骑手（只适合一个task的骑手）
3. 低意愿场景：更激进的backup策略，每个slot分配更多骑手
4. 顺序接单模型优化：骑手排序影响期望成本，按score*w升序排列
"""
...
def _slot_cost_sequential(tk, cids, cand_map, penalty):
    """v22: 顺序接单模型的slot成本
    骑手按顺序尝试接单，第一个接受则匹配成功。
    E[cost] = sum_k [P(前k-1拒单) * w_k * score_k] + P(全部拒单) * penalty * n
    骑手排序影响期望成本：最优排序是按score升序（低score优先尝试）。
    """
    if not cids:
        return penalty * 100
    first_c = cand_map.get((tk, cids[0]))
    ...
    for cid in sorted_cids:
        ...
        if c:
            w = c["willingness"]
            s = c["score"]
            expected_score += p_all_rej * w * s
            p_all_rej *= (1 - w)
    return expected_score + p_all_rej * penalty * n
...
def _solve_main(input_text: str) -> list:
    ...
    if is_low_w:
        # v22: 低意愿场景增加collision_aware策略
        strategies = ("global", "hard_first", "doubles_first", "willingness_first", "collision_aware")
    ...
    for primary_strategy in strategies:
        for penalty in penalties:
            ...
            elif primary_strategy == "collision_aware":
                primary = _pick_primary_collision_aware(candidates, all_tasks, taskkey_to_cands, penalty)
            else:
            ...
            # v22: 低意愿时用更高penalty评估backup（让backup更激进）
...
def _pick_primary_collision_aware(candidates, all_tasks, taskkey_to_cands, penalty):
    """v22: 碰撞感知策略（基于MIECP论文）
    核心思想：优先选择"独占"骑手（只适合当前task的骑手），
    因为这些骑手不会被其他task抢走。
    同时考虑骑手的"竞争度"——一个骑手适合的task越多，被抢走的概率越高。
    """
    import math

    # 计算每个骑手的竞争度（能匹配多少个不同的task_key）
    courier_task_count = defaultdict(int)
    for c in candidates:
        courier_task_count[c["courier_id"]] += 1

    # 计算每个task的候选骑手数（稀缺度）
    task_cand_count = {}
    for tk, cands in taskkey_to_cands.items():
        task_cand_count[tk] = len(cands)

    # 按task稀缺度排序（候选少的task优先），然后按骑手独占度排序
    enriched = []
    for c in candidates:
        n = len(c["task_ids"])
        w = c["willingness"]
        per_task_cost = (w * c["score"] + (1 - w) * penalty * n) / n
        # 独占度：骑手只适合1个task时独占度最高
        exclusivity = 1.0 / courier_task_count[c["courier_id"]]
        # 综合评分：独占度越高越好（排前面），cost越低越好
        # 用指数锥近似的思想：log(1/(1-w))近似willingness的贡献
        log_w = -math.log(1 - w + 1e-10)  # w越高，log_w越大
        # 排序key：独占度优先，然后是willingness贡献，最后是cost
        enriched.append((-exclusivity, -log_w, per_task_cost, c))

    enriched.sort(key=lambda x: (x[0], x[1], x[2]))
    used_c = set()
    used_t = set()
    out = []
    for _, _, _, c in enriched:
        if c["courier_id"] in used_c:
            continue
        if any(t in used_t for t in c["task_ids"]):
            continue
...
def _add_backups(primary, all_couriers, taskkey_to_cands, cand_map, penalty, aggressive=False):
    ...
    min_p_rej = 0.001 if aggressive else 0.03
    max_rounds = 12 if aggressive else 3
